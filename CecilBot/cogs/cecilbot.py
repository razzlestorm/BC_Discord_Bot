...
RANDOM_SKILLSETS_TABLE = path.join(tblpath, "Random Skillsets v2.2.xls")
...
class DiscordCecilBot(commands.Cog):
    ...
    async def lookup(self, interaction: discord.Interaction, query: str) -> None:
        ...
        await interaction.response.send_message(
            f"{interaction.user.name}, I've recorded the following activity:"
    ...
    def command_lookup(self, author, message):
        ...
        if rchaospattern.search(message):
            ...
            try:
                ...
                correction = generate_correction(temp, data.skill_parameters)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
        elif bosspattern.search(message):
            try:
                temp = re.sub("!boss ", "", message, flags=re.IGNORECASE).lower()
                return (data.boss_moves[temp])
            except:
                correction = generate_correction(temp, data.boss_moves)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.codes)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.item_table)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.tools)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.special_equipment)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.special_weapons)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.status_effects)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
                return f"Sorry, could not find {temp}. Please check your spelling and try again."
            ...
            try:
                ...
                correction = generate_correction(temp, data.root_table)
                if correction:
                    return f"Sorry, could not find {temp}. Did you mean {correction}?"
    ...
    t        return "That command didn't work, check your spelling and try again!"
    """
